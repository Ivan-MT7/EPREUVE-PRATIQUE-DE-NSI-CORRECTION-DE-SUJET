...
class Piece:
    ...
    def nb_occupants_restants(self):
        ''' renvoie le nombre d'occupants restants dans la pièce.
            A FAIRE (QUESTION 1)
        '''
        # initialisation de la variable qui contiendra la somme totale
        compteur = 0
        # Avec la variable etage, on parcourt les listes
        for ligne in self.grille :
            #Avec la variable personne les éléments de la liste
    ...
    def ajouter_sortie(self, direction, position):
        ...
        if direction == "N":
            ...
            # on ajoute le cas de l'Est "E", la colonne correspondra tout à droite (self.j_max)
            self.sorties.append((position, self.j_max))
        elif direction == "S" :
            # on ajoute le cas du Sud "S", la ligne correspondra tout en bas (self.i_max)
    ...
    def choix_sortie(self, i, j):
        ...
        for k in range(1, len(self.sorties)):
            ...
            #On définit également une variable nouveau_distance pour pouvoir comparer
            nouveau_distance = abs(i - autre_sortie[0]) + abs(j - autre_sortie[1])
            # l'erreur logique était k < 0, on la remplace par la comparaison de distance : si d2 est plus courte
            if nouveau_distance < distance:
                choix = autre_sortie
                distance = nouveau_distance
        return choix
...
def evacuation(p, silencieux=True):
    ...
    '''
    # on initialise la variable pour compter le nombre de tours à 0
    tours = 0
    # on répète l'action de déplacer les gens (alerter) tant qu'il reste quelqu'un
    while p.nb_occupants_restants() > 0:
        p.alerter(silencieux)
        # un tour vient de se dérouler, donc on l'ajoute au compteur
        tours  += 1
        # si on nous demande le mode silencieux, alors on affiche la pièce à ce tour-ci
        if silencieux == False :
            print(p)
    # on retourne le nombre de tours total
    return tours
