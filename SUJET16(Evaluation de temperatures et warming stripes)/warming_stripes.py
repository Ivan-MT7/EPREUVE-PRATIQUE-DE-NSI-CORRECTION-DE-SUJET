...
def ecart_temperature(datas, annee) :
    # Parcours complet de la liste des dictionnaires (élément par élément)
    for donnee in datas :
        # On vérifie si l'année du dictionnaire actuel correspond à l'année cherchée
        if donnee["année"] == annee :
            # Dès qu'on trouve l'année, on renvoie son écart et on quitte la fonction
            return donnee["écart"]
    # Si on arrive ici, celà signifie que la boucle s'est terminée sans trouver l'année

    return None
def test_ecart_temperature() :
    assert ecart_temperature(datas_temperature, 1851) == -0.15
    assert ecart_temperature(datas_temperature, None) == None
    assert ecart_temperature(datas_temperature, 2003) == 0.62
    print("Test réussi")    
...
def derniere_annee_ecart_negatif(datas):
    ...
    while ecart >= 0:
        ...
        ecart = ecart_temperature(datas, annee)
    return annee
# Appel purement consultatif pour la question 2 (détermination de la dernière année d'écart négatif)
...
#La derniere annee où la moyenne était inférieure était en 1977
...
def moyenne_ecarts(annee_debut, annee_fin, datas):
    ...
    for dico in datas:
        if annee_debut <= dico["année"] and dico["année"] <= annee_fin:
            # CORRECTION : l'erreur logique était une soustraction au lieu d'une addition.
            # Pour calculer une moyenne, il faut bien additionner au fur et à mesure toutes les valeurs.
            somme = somme + dico["écart"] # Modifié : somme = somme - dico["écart"] deviens une addition (+)
            compteur += 1
...
def graphique(datas):
    ...
    annees = [dico["année"] for dico in datas] #On extrait toutes les années de la liste
    nombre_elements = len(annees) #On compte le nombre d'années dans cette liste
    ordonnees = [1 for i in range(nombre_elements)] #On ajoute autant de 1
